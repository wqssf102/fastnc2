...
int main(int argc, char **argv) {
	...
	FastNC fastnc(fastnc_options.adj_table_filename, fastnc_options.outfile_filename, fastnc_options.threshold_name, fastnc_options.number_name, fastnc_options.jobs_name, fastnc_options.edge_name, fastnc_options.step_name);
	...
	if(fastnc_options.step_name>1){
	int mtrows_tt_tmp = ceil(data_M.rows()*fastnc_options.threshold_name);
	int mtrows_tt = mtrows_tt_tmp / fastnc_options.step_name;
	if((mtrows_tt_tmp-(mtrows_tt * fastnc_options.step_name)) > 0)
	{
		mtrows_tt = mtrows_tt + 1;
	}
	data_R.setZero(mtrows_tt,2);
	...
	{
		printf("\tRunning %d\n", i);
		data_R = data_R + natural_connectivity_only_node_step(data_M, fastnc_options.threshold_name, fastnc_options.step_name);
	}}else{
		omp_set_num_threads(fastnc_options.jobs_name);
		#pragma omp declare reduction( + : Eigen::MatrixXd : omp_out += omp_in ) initializer( omp_priv = omp_orig )
		#pragma omp parallel for reduction(+:data_R), schedule(static, 1)
	...
	{
		printf("\tRunning %d\n", i);
		data_R = data_R + natural_connectivity_step(data_M, fastnc_options.threshold_name,fastnc_options.edge_name, fastnc_options.step_name);
	}}
	}else{
		data_R.setZero(ceil(data_M.rows()*fastnc_options.threshold_name),2);
		if(fastnc_options.edge_name==0){
...
FastNC::FastNC(const std::string &inputfile, const std::string &outfile, const float &_threshold, const int &_number, const int &_jobs, const float &_edge, const int &_step) {
...
//
MatrixXd natural_connectivity_step(const MatrixXd& re_matrix,  const float del_per, const float del_edge_per,  const int step)
{
    int total_nodes = re_matrix.rows();//获取原始矩阵的行数
	MatrixXd resdt = Matrix<double, Dynamic, 2>();//定义一个 total_nodes行、2列的矩阵，用来存放最后计算结果
	int mtrows_tmp = ceil(total_nodes * del_per) ;
	int mtrows = mtrows_tmp / step ;
	if(mtrows < 2)
	{
	...
	//
	if((mtrows_tmp-(mtrows * step)) > 0)
	{
		mtrows = mtrows + 1;
	}
	
	resdt.setZero(mtrows, 2);//对定义的矩阵初始化
	//2 获取随机数
	int del_total_nodes = ceil(total_nodes * del_per) - 1 ;
for (int rmnd = 0; rmnd <= del_total_nodes; rmnd=rmnd+step)
	...
	if (rmnd + step > del_total_nodes)
			{
			resdt((resdt.rows())-1, 0) = get_nc(matrix, total_nodes);
		    resdt((resdt.rows())-1, 1) = (rmnd + 1) * 1.0 / total_nodes * 1.0;
			}
		resdt(rmnd/step, 0) = get_nc(matrix, total_nodes);
...
MatrixXd natural_connectivity_only_node_step(const MatrixXd& re_matrix,  const float del_per,  const int step)
{
	int total_nodes = re_matrix.rows();//获取原始矩阵的行数
	MatrixXd resdt = Matrix<double, Dynamic, 2>();//定义一个 total_nodes行、2列的矩阵，用来存放最后计算结果
	int mtrows_tmp = ceil(total_nodes * del_per) ;
